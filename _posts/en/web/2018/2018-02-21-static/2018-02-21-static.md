...
{% capture img_caption %}A high TTFB is likely to penalize the Speed Index of a page.{% endcapture %}
{% include rwd-image.html.liquid 
...
After the contribution of the contents, the generation of the website and the deployment are often operated by the same component of the infrastructure. The duration of the generation and deployment and the CPU consumption both locally and on the targeted infrastructure (composed of one or more servers) must be measured. From a system where the main metric is the number of simultaneous visitors, DevOps must switch their mindset to create a system that is able to sustain the necessary frequency of contribution, generation, and deployment.
...
JAMStack is a real paradigm shift. The website is no longer a complete code base in which libraries are imported. It becomes a shell hub in which self-hosted or third-party services are dynamically injected. It is even possible to rely on several services for the same purpose and switch to a fallback if the main service is not available.

For the user, however, the difference is small. Try to search a product [on this website](https://community.algolia.com/instantsearch.js/v2/examples/e-commerce/). Do you experience something different than from a PHP website? And you can go further: Your websites can become complete Progressive Web Apps built with a Single Page App framework such as Vue, Angular or React, designed to be offline first.

For contributors, the paradigm shift changes everything as it separates the contribution platform from the contributed website. Some of them will prefer to use software that features writing aids (correction, suggestions, documentary contributions). Others will be satisfied with customizable online contribution platforms. Assuming that the storage of the content is independent of the solution, each of them can use their favorite tool without interfering with others’.
...
The first risk is to get lost in the plethora of Headless CMS, generators and service platforms. Take the time to evaluate your needs because each solution has its advantages and drawbacks. Jekyll, for example, is a well-known SSG, developed in Ruby, well documented but fairly slow. Hugo, on the other hand, is a much faster SSG but is also more complex to handle if you are a novice. If you don't publish content at high-frequency, does the generation time matter so much, for an equivalent result?
...
{% capture img_alt %}Dareboost graph of the TTFB of three locations{% endcapture %}
{% capture img_caption %}TTFB of a website hosted by Netlify from Tokyo, Washington, Frankfurt. The indicator may seem unstable but look at the y-axis: oscillations don’t exceed 200ms, and  are very punctual.{% endcapture %}
{% include rwd-image.html.liquid 
    path="/assets/images/2018-02-21/5_ttfb_netlify_en.png"
    alt=img_alt
    caption=img_caption
%}
...
_Thanks to Erin Symons, [Frank Taillandier](https://twitter.com/dirtyf) and the whole [jamstatic.fr community](https://jamstatic.fr/), [Bud Parr](https://twitter.com/budparr), [Nicolas Hoffman](https://twitter.com/Nico3333fr) and my colleague [Philippe Guibert](https://twitter.com/GuilbertPhil) for their time and advice._
